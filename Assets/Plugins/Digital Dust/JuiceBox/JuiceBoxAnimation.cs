...
namespace JuiceBox
{
    ...
    /// </summary>
    public class JuiceBoxAnimation : Processor, ISerializationCallbackReceiver, IAnimationEditorComponent
    {
#if UNITY_EDITOR
        ...
        private static void MigrateAnimationData(JuiceBoxAnimation anim, uint fromVersion, uint toVersion)
        {
        }

        public void OnBeforeSerialize()
        {
            if (_dataVersion != DATA_VERSION)
            {
                MigrateAnimationData(this, _dataVersion, DATA_VERSION);
        ...
        public bool IsRunning(string sequenceName)
        {
            ...
            return JuiceBoxCentralController.IsSequenceRunning(seq);
        }
        ...
        /// <summary>Pauses the first sequence whose Name matches. Tries an exact match
        /// first, then a case-insensitive match.</summary>
        public void Pause(string sequenceName)
        {
            var seq = FindSequenceByName(sequenceName);
            if (seq != null) { PauseSequence(seq); return; }
        ...
        /// <summary>Resumes the first sequence whose Name matches. Tries an exact match
        /// first, then a case-insensitive match.</summary>
        public void Resume(string sequenceName)
        {
            var seq = FindSequenceByName(sequenceName);
            if (seq != null) { ResumeSequence(seq); return; }
            Debug.LogWarning($"JuiceBoxAnimation: no sequence named \"{sequenceName}\" found.", this);
        }
        ...
        /// <summary>Starts the first sequence whose Name matches. Tries an exact match
        /// first, then a case-insensitive match.</summary>
        public void StartSequence(string sequenceName)
        {
            var seq = FindSequenceByName(sequenceName);
            if (seq != null) { PlaySequence(seq); return; }
            Debug.LogWarning($"JuiceBoxAnimation: no sequence named \"{sequenceName}\" found.", this);
        }
        ...
        /// <summary>Stops the first sequence whose Name matches. Tries an exact match
        /// first, then a case-insensitive match.</summary>
        public void StopSequence(string sequenceName)
        {
            var seq = FindSequenceByName(sequenceName);
            if (seq != null) { StopSequence(seq); return; }
            Debug.LogWarning($"JuiceBoxAnimation: no sequence named \"{sequenceName}\" found.", this);
        }

        /// <summary>Finds the first sequence whose Name matches, trying an exact
        /// (case-sensitive) match first and falling back to a case-insensitive match.
        /// Returns null if none match.</summary>
        private Sequence FindSequenceByName(string sequenceName)
        {
            ...
            for (int i = 0; i < Sequences.Count; i++)
                if (Sequences[i].Name == sequenceName)
                    return Sequences[i];
            for (int i = 0; i < Sequences.Count; i++)
                if (string.Equals(Sequences[i].Name, sequenceName,
