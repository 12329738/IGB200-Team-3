...
namespace JuiceBox
{
    ...
    {
        static JuiceBoxSettings()
        {
            if (SessionState.GetBool(MigrationProbedKey, false)) return;
            SessionState.SetBool(MigrationProbedKey, true);
            EditorApplication.delayCall += MigrateLegacySettings;
        }

        private const string MigrationProbedKey = "JuiceBox.SettingsMigrationProbed";
        ...
        private static string SettingsPath
        {
            get
            {
                ...
                _settingsPath = System.IO.Path.Combine(root, "ProjectSettings",
                   "JuiceBoxSettings.json").Replace('\\', '/');
                return _settingsPath;
            }
        }

        // == MIGRATION -- added 2026-07, delete once the user base has cycled ==========
        // LegacySettingsPath, MigrateLegacySettings and the delayCall in the static
        // constructor exist only to move a settings file written by an earlier release,
        // which stored it beside this script. They go together. Removing them early is
        // not destructive: the stale file is left behind and that project falls back to
        // defaults.
        ...
        private static string LegacySettingsPath()
        {
            ...
            foreach (string guid in guids)
            {
                ...
                if (!assetPath.EndsWith("JuiceBoxSettings.cs")) continue;
                int slash = assetPath.LastIndexOf('/');
                return assetPath.Substring(0, slash) + "/JuiceBoxSettings.json";
            }
            return "Assets/JuiceBox/Editor/JuiceBoxSettings.json";
        }
        ...
        private static void MigrateLegacySettings()
        {
            ...
            try
            {
                _data = ReadFile(legacy);
                if (_data != null) Save();

                AssetDatabase.DeleteAsset(legacy);   // takes the .meta with it
                Debug.Log("JuiceBox: moved settings from " + legacy
                   + " to ProjectSettings/JuiceBoxSettings.json.");
            }
            catch (Exception ex)
            {
                Debug.LogWarning("JuiceBox: could not migrate the old settings file at "
                   + legacy + ": " + ex.Message + ". Defaults will be used.");
            }
        }
        ...
        private static SettingsData ReadFile(string path)
        {
            ...
            {
                Debug.LogWarning("JuiceBox: could not read settings at " + path
                   + ": " + ex.Message + ". Defaults will be used.");
                return null;
            }
        }
    ...
    internal sealed class JuiceBoxSettingsWindow : EditorWindow
    {
        ...
        {
            JuiceBoxSettings.InvalidateCache();
