...
namespace JuiceBox
{
    ...
    public static class SequenceBackupManager
    {
        ...
        public static bool RestoreSnapshot(Sequence sequence, int index, Object dirtyTarget)
            => RestoreSnapshot(sequence, index, dirtyTarget, out _);
            ...
            out string error)
        {
            ...
            { error = "The sequence has no name."; return false; }
            string filePath = GetFilePath(sequence.Name);
            if (filePath == null || !File.Exists(filePath))
            { error = $"No snapshot file was found for \"{sequence.Name}\"."; return false; }

            BackupFile bf = ReadFile(filePath);
            if (bf.snapshots == null || index < 0 || index >= bf.snapshots.Count)
            { error = "That snapshot is no longer available."; return false; }

            string json = bf.snapshots[index].sequenceJson;
            if (string.IsNullOrEmpty(json))
            { error = "The snapshot is empty or could not be read."; return false; }

            List<string> objIds = ExtractObjIds(json);
            JuiceBoxEditorHooks.WarmObjectIds(objIds);

            sequence.Property = null;
        ...
        private static List<string> ExtractObjIds(string json)
        {
            var result = new List<string>();
            if (string.IsNullOrEmpty(json)) return result;

            const string key = "\"ObjId\"";
            int i = 0;
            while (true)
            {
                int k = json.IndexOf(key, i, System.StringComparison.Ordinal);
                if (k < 0) break;
