...
// ==============================================================================
//  JBGraphView.PathRouter: Channel graph, A* routing, lane separation, and per-port exit directions.
// ==============================================================================
namespace JuiceBox
{
    internal sealed partial class JBGraphView
    {
        ...
        private const float ChannelSpread = 24f;
        private const float LexEps = 0.25f;
        private const float EdgeBiasFraction = 0.9f;
        ...
        private void BuildChannelGraph()
        {
            ...
            if (_vUse == null || _vUse.Length < _gV) _vUse = new float[_gV];
            else System.Array.Clear(_vUse, 0, _gV);
            if (_hUse == null || _hUse.Length < _gH) _hUse = new float[_gH];
            else System.Array.Clear(_hUse, 0, _gH);
        ...
        private bool RouteThroughChannels(Vector2 po, Vector2 pi, Dir dOut, Dir dIn, List<Vector2> outPts)
        {
            ...
            for (int i = 0; i < total; i++) { _g[i] = float.MaxValue; _g2[i] = float.MaxValue; _from[i] = -1; _closed[i] = false; }
            _routeDst = pi;
            ...
            while (_open.Count > 0)
            {
                ...
                for (int i = 0; i < _open.Count; i++)
                {
                    ...
                    float f = _g[nd] + h;
                    if (f < bf - LexEps || (f <= bf + LexEps && _g2[nd] < bf2)) { bf = f; bf2 = _g2[nd]; bi = i; }
                }
                int cur = _open[bi];
                ...
                if (cur == S)
                {
                    for (int i = 0; i < _srcConn.Count; i++)
                        Relax(S, _srcConn[i].node, _srcConn[i].cost, 0f);
                }
                else
                {
                    ...
                    if (nb != null)
                    {
                        ...
                        {
                            int to = nb[i];
                            float seco = (to / _gH == cvi) ? _vUse[cvi] : _hUse[chi];
                            Relax(cur, to, (NodePos(to) - cp).magnitude, seco);
                        }
                    }
                    for (int i = 0; i < _dstConn.Count; i++)
                        if (_dstConn[i].node == cur)
                            Relax(cur, T, _dstConn[i].cost, 0f);
                }
            }
        ...
        private void Relax(int from, int to, float cost, float seco)
        {
            float ng = _g[from] + cost;
            float ng2 = _g2[from] + seco;
            float d = ng - _g[to];
            if (d < -LexEps || (d <= LexEps && ng2 < _g2[to]))
            {
                if (ng < _g[to]) _g[to] = ng;
                _g2[to] = ng2;
                _from[to] = from;
        ...
        private void BumpUsage()
        {
            ...
            for (int i = 1; i < _path.Count; i++)
            {
                ...
                if (a / _gH == b / _gH) BleedV(a / _gH);
                else if (a % _gH == b % _gH) BleedH(a % _gH);
            }
        }

        private void BleedV(int vi)
        {
            float cx = _vxs[vi];
            for (int k = vi; k >= 0; k--)
            {
                float dd = cx - _vxs[k];
                if (dd > ChannelSpread) break;
                _vUse[k] += 1f - dd / ChannelSpread;
            }
            for (int k = vi + 1; k < _gV; k++)
            {
                float dd = _vxs[k] - cx;
                if (dd > ChannelSpread) break;
                _vUse[k] += 1f - dd / ChannelSpread;
        ...
        private void BleedH(int hi)
        {
            ...
            for (int k = hi; k >= 0; k--)
            {
                float dd = cy - _hys[k];
                if (dd > ChannelSpread) break;
                _hUse[k] += 1f - dd / ChannelSpread;
            }
            for (int k = hi + 1; k < _gH; k++)
            {
                float dd = _hys[k] - cy;
                if (dd > ChannelSpread) break;
                _hUse[k] += 1f - dd / ChannelSpread;
        ...
        private bool ChannelHugsStripRight(int q, float width)
        {
            if (_strips == null) return false;
            float rightEdge = q + width * 0.5f;
            for (int s = 0; s < _strips.Count; s++)
            {
                Rect sp = _strips[s].GetPosition();
                if (Mathf.Abs(rightEdge - (sp.x + sp.width)) < 2f) return true;
            }
            return false;
        }
        ...
        private void AssignLane(int q, List<int> wires, float width, bool vertical)
        {
            ...
            float bias = span * 0.5f * EdgeBiasFraction - (lanes - 1) * 0.5f * pitch;
            float neutral = vertical ? (ChannelHugsStripRight(q, width) ? -bias : bias) : 0f;
