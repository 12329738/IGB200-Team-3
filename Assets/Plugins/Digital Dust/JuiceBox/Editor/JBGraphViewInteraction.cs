...
// ==============================================================================
//  JBGraphView.Interaction: Drag, snap, edge wiring, and layout persistence for the graph view.
// ==============================================================================
namespace JuiceBox
{
    internal sealed partial class JBGraphView
    {
        ...
        internal void ApplySequence(int si, bool broadcast = true)
        {
            ...
            for (int i = 0; i < ordered.Count; i++)
            {
                ...
                if (node.SlotIndex != expectedSlot) break;
        ...
        private void NotifyLibraryChanged(string name, Sequence seq)
        {
            ...
            if (_window != null) _window.UpdateSequenceBaseline(name, seq);
        ...
        private void NotifyLibraryRenamed(string oldName, string newName, Sequence seq)
        {
            ...
            if (_window != null)
            {
                _window.RemoveSequenceBaseline(oldName);
                _window.UpdateSequenceBaseline(newName, seq);
            }
            if (_window != null) _window.SuppressLibraryRebuild = false;
        }
