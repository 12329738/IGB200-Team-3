...
// ==============================================================================
//  JBGraphView.Operations: High-level operations: add/remove sequences, rename, reorder, context menus.
// ==============================================================================
namespace JuiceBox
{
    internal sealed partial class JBGraphView
    {
        ...
        private void OnSequenceNameChanged(int si, string newName)
        {
            ...
            if (nameError != null)
            {
                _window?.SetMessage(nameError,
                    SequenceEditorWindow.MessageSeverity.Error);
                return;
            }
            ...
            _window?.SetMessage("", SequenceEditorWindow.MessageSeverity.Error);
        ...
        private void FlushPendingRename(int si)
        {
            ...
            if (!string.IsNullOrEmpty(newName))
            {
                ...
                {
                    if (i == si) continue;
                    if (_anim.Sequences[i].Name != newName) continue;
                    _window?.SetMessage(
                        $"This animation already has a sequence named \"{newName}\". Names must be unique within an animation.",
                        SequenceEditorWindow.MessageSeverity.Error, 10f);
                    ResetStripDisplayName(si, name);
                    return;
                }
            }
        ...
        private void ResetStripDisplayName(int si, string name)
        {
            if (si < _strips.Count)
        ...
        internal void OnSeqHookPickerClicked(int si, string slotName, Vector2 pos,
            GameObject prePickedTarget)
        {
            if (_anim == null || si < 0 || si >= _anim.Sequences.Count) return;
            var seq = _anim.Sequences[si];
            if (seq.Property == null) return;
            ...
            DelegatePicker.Show(
                this,
                (IDelegateConnecter)seq,
                slotName,
                PropertyTypeToSystemType(seq.Type),
                this.WorldToLocal(pos),
                _theme,
                () => CommitSequenceHook(seq, slotName, recordUndo: true),
                () =>
                {
                    if (_anim != null) Undo.RecordObject(_anim, "Clear " + slotName);
                    seq.WriteSlot(slotName, (int)DelegateMode.None, null, "", "", "");
                    seq.Reconstruct();
                    CommitSequenceHook(seq, slotName, recordUndo: false);
                },
                selfObject: _anim != null ? _anim.gameObject : null,
                prePickedTarget: prePickedTarget);
        }

        /// <summary>Finishes a sequence-hook edit. Takes the sequence rather than its index: the picker
        /// is modeless, so the strip it was opened on can be closed or the whole target swapped before
        /// the callback fires, and an index captured at open time would then name a different sequence.
        /// recordUndo is false on the delete path, which records before clearing so the undo entry holds
        /// the binding rather than the cleared slot.</summary>
        private void CommitSequenceHook(Sequence seq, string slotName, bool recordUndo)
        {
            if (_anim == null || seq == null) return;

            int si = _anim.Sequences.IndexOf(seq);
            if (si < 0) return;

            if (recordUndo) Undo.RecordObject(_anim, "Set " + slotName);
            EditorUtility.SetDirty(_anim);
            SequenceLibrary.PropagatePropertySlot(seq.Name, seq, slotName, _anim);
            RefreshLeftCapPickers(si);
        ...
        internal void RefreshLeftCapPickers(int si)
        {
            ...
            _strips[si].PaintSequenceHookRows();
        }
        ...
        public void LoadSequence(Sequence seq)
        {
            ...
            if (!string.IsNullOrEmpty(seq.Name))
                for (int i = 0; i < _anim.Sequences.Count; i++)
                    if (_anim.Sequences[i].Name == seq.Name)
                    {
                        _window?.SetMessage(
                            $"This animation already has a sequence named \"{seq.Name}\". Names must be unique within an animation.",
                            SequenceEditorWindow.MessageSeverity.Error, 10f);
                        return;
                    }
        ...
        public void AddSequence(Vector2 canvasPos, PropertyTypes type = PropertyTypes.Vector3)
        {
            ...
            var strip = new FilmStripElement(seq, origin, initialSlots, si, _anim.Sequences.Count, _theme, OnSequenceNameChanged, OnSegmentChanged, this, RemoveStrip, OnTriggersChanged, OnLeftCapInitialClicked, OnLeftCapUpdateClicked, MoveStrip, OnSeqHookPickerClicked);
            AddElement(strip);
            _strips.Add(strip);
        ...
        internal void RemoveEffectNode(EffectNode node)
        {
            ...
            if (stripValid)
            {
                ...
                if (slot >= 0 && slot < _occ[si].Count && _occ[si][slot] == node)
                    _occ[si][slot] = null;
            }
            ...
            for (int i = _smoothingNodes.Count - 1; i >= 0; i--)
            {
                ...
                if (sn.SourceEffect != node.SourceEffect) continue;
        ...
        private void RemoveStrip(FilmStripElement strip)
        {
            ...
            string removedName = si < _anim.Sequences.Count ? _anim.Sequences[si].Name : null;
            ...
            string undoKey = Application.platform == RuntimePlatform.OSXEditor ? "Cmd+Z" : "Ctrl+Z";
            string what = string.IsNullOrWhiteSpace(removedName)
                ? "sequence" : $"sequence \"{removedName}\"";
            _window?.SetMessage($"Removed {what}. Undo with {undoKey} to restore it.",
                SequenceEditorWindow.MessageSeverity.Info, 10f);
            _window?.RefreshEmptyHint();
        }
