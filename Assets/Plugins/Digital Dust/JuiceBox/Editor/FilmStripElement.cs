...
namespace JuiceBox
{
    internal sealed class FilmStripElement : GraphElement
    {
        ...
        private readonly System.Action<int, string, Vector2, GameObject> _onSeqHookPickerClicked;
        ...
        private VisualElement _completeBadgeBg;
        private Label _completeBadgeLbl;
        private VisualElement _completeBadgeRoot;
        private VisualElement _finallyBadgeBg;
        private Label _finallyBadgeLbl;
        private VisualElement _finallyBadgeRoot;
        ...
        public FilmStripElement(Sequence sequence, Vector2 canvasOrigin, int slotCount,
           ...
           System.Action<int, string, Vector2, GameObject> onSeqHookPickerClicked = null)
        {
            _theme = theme.Initialized ? theme : JbTheme.Default;
            _sequence = sequence;
            ...
            _onSeqHookPickerClicked = onSeqHookPickerClicked;

            capabilities = 0;
            pickingMode = PickingMode.Ignore;

            style.backgroundColor = _theme.StripBg;
            style.backgroundImage = Background.FromTexture2D(MottleTexture());
            style.backgroundSize = new BackgroundSize(new Length(MottleW), Length.Percent(100f));
            style.backgroundRepeat = new BackgroundRepeat(Repeat.Repeat, Repeat.NoRepeat);
            style.unityBackgroundImageTintColor = _theme.StripSpeckle;
        ...
        // -- Background mottle -----------------------------------------------------
        // Tiled horizontally at native width so the grain does not rescale with
        // strip length. MottleW must stay a whole multiple of MottleCell.
        private const int MottleW = 500;
        private const int MottleCell = 100;
        private const int MottleOctaves = 3;
        private static Texture2D s_mottle;

        private static uint MottleHash(int x, int y)
        {
        ...
        private static float MottleLattice(int x, int y, int px, int py)
        {
            x = ((x % px) + px) % px;
            y = ((y % py) + py) % py;
            return (MottleHash(x, y) & 0xFFFFu) * (1f / 65535f);
        }

        private static float MottleNoise(float x, float y, int px, int py)
        {
            int x0 = Mathf.FloorToInt(x), y0 = Mathf.FloorToInt(y);
            float fx = x - x0, fy = y - y0;
            fx = fx * fx * (3f - 2f * fx);
            fy = fy * fy * (3f - 2f * fy);
            float a = Mathf.Lerp(MottleLattice(x0, y0, px, py),
               MottleLattice(x0 + 1, y0, px, py), fx);
            float b = Mathf.Lerp(MottleLattice(x0, y0 + 1, px, py),
               MottleLattice(x0 + 1, y0 + 1, px, py), fx);
            return Mathf.Lerp(a, b, fy);
        }

        private static Texture2D MottleTexture()
        {
            if (s_mottle != null) return s_mottle;

            int h = Mathf.RoundToInt(StripTotalHeight);
            var tex = new Texture2D(MottleW, h, TextureFormat.RGBA32, false)
            {
                name = "JuiceBoxStripMottle",
                hideFlags = HideFlags.HideAndDontSave,
                wrapMode = TextureWrapMode.Repeat,
                filterMode = FilterMode.Bilinear,
            };

            int basePx = Mathf.Max(2, MottleW / MottleCell);
            int basePy = Mathf.Max(2, Mathf.RoundToInt((float)h / MottleCell));
            var px = new Color32[MottleW * h];
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < MottleW; x++)
                {
                    float sum = 0f, total = 0f, amp = 1f;
                    int pX = basePx, pY = basePy;
                    for (int o = 0; o < MottleOctaves; o++)
                    {
                        sum += MottleNoise(x * (float)pX / MottleW, y * (float)pY / h, pX, pY) * amp;
                        total += amp;
                        amp *= 0.5f;
                        pX *= 2;
                        pY *= 2;
                    }
                    int a = Mathf.Clamp(Mathf.RoundToInt(sum / total * 255f), 0, 255);
                    px[y * MottleW + x] = new Color32(255, 255, 255, (byte)a);
                }
            }
        ...
        public void SetTargetSlot(int slot, bool denied = false)
        {
            ...
            for (int i = 0; i < _slotBoxes.Count; i++)
            {
                ...
                _slotBoxes[i].style.backgroundColor = hi
                    ? (denied ? _theme.SlotBgDenied : _theme.SlotBgHi)
                    : _theme.StripBg;
            }
        }
        ...
        private void BuildLeftCap()
        {
            ...
            BuildCapRestoreButton(cap, lp, y); y += 32f;
            BuildCapSequenceHookRows(cap, lp, ref y);
        ...
        internal const string CompleteTooltip =
            "Fires once when the sequence finishes normally. A run cut short by its GameObject being "
            + "destroyed or deactivated does not count -- use Finally for cleanup that has to happen "
            + "either way";
        internal const string FinallyTooltip =
            "Fires when the sequence ends, however it ends: completed, stopped, its GameObject "
            + "destroyed or deactivated, or killed by an exception out of your own callback. Fires "
            + "exactly once per run, one frame after the run stops";

        private void BuildCapSequenceHookRows(VisualElement cap, float lp, ref float y)
        {
            cap.Add(HRule(lp, y, LeftCapW - lp * 2f)); y += 7f;

            BuildCapHookRow(cap, "OnComplete", CompleteTooltip, lp, y,
                out _completeBadgeRoot, out _completeBadgeBg, out _completeBadgeLbl); y += 28f;
            BuildCapHookRow(cap, "Finally", FinallyTooltip, lp, y,
                out _finallyBadgeRoot, out _finallyBadgeBg, out _finallyBadgeLbl); y += 28f;

            PaintSequenceHookRows();
        }

        /// <summary>One sequence-level hook row: the slot name on the left half, the delegate picker on
        /// the right half. Unlike ON UPDATE and INITIAL VALUE the name is a plain label rather than part
        /// of the picker, so the two rows read as a pair without repeating a heading for each.</summary>
        private const float HookRowPickerBonus = 10f;

        private void BuildCapHookRow(VisualElement cap, string slotName, string tooltip,
            float x, float y, out VisualElement root, out VisualElement chip, out Label label)
        {
            float full = LeftCapW - x * 2f;
            float half = Mathf.Floor(full * 0.5f) - HookRowPickerBonus;

            var nameLbl = new Label(slotName);
            nameLbl.tooltip = tooltip;
            nameLbl.style.position = Position.Absolute;
            nameLbl.style.left = x;
            nameLbl.style.top = y + 4f;
            nameLbl.style.width = half - 2f;
            nameLbl.style.fontSize = 13f;
            nameLbl.style.color = _theme.CapLabel;
            nameLbl.style.whiteSpace = WhiteSpace.NoWrap;
            nameLbl.style.overflow = Overflow.Hidden;
            nameLbl.style.textOverflow = TextOverflow.Clip;
            cap.Add(nameLbl);

            (root, chip, label) = BuildPickerChrome(x + half, y, full - half, "\u2014 unset \u2014");
            ...
            root.RegisterCallback<MouseDownEvent>(evt =>
            {
                ...
                evt.StopPropagation();
                _onSeqHookPickerClicked?.Invoke(
                    capturedStripIndex, slotName, evt.originalMousePosition, null);
            });

            DelegatePicker.RegisterDragToPick(root, _theme, (droppedGo, mousePos) =>
                _onSeqHookPickerClicked?.Invoke(
                    capturedStripIndex, slotName, mousePos, droppedGo));

            cap.Add(root);
        }

        /// <summary>Repaints both hook rows from the sequence itself, so a caller that changed either one
        /// does not have to know which.</summary>
        public void PaintSequenceHookRows()
        {
            PaintHookRow(_completeBadgeRoot, _completeBadgeBg, _completeBadgeLbl, "OnComplete");
            PaintHookRow(_finallyBadgeRoot, _finallyBadgeBg, _finallyBadgeLbl, "Finally");
        }

        private void PaintHookRow(VisualElement root, VisualElement chip, Label label, string slotName)
        {
            if (_sequence == null) return;
            var conn = (IDelegateConnecter)_sequence;
            var slot = conn.ReadSlot(slotName);
            bool live = slot.mode == 0 || conn.GetLiveDelegate(slotName) != null;
            PaintInitialPicker(root, chip, label, true,
                slot.mode, slot.method ?? "", slot.relDesc ?? "", live);
        }
        ...
        private void BuildCapNameField(VisualElement cap, float lp, ref float y)
        {
            ...
            nameLabel.style.color = _theme.CapLabel;
            cap.Add(nameLabel);
        ...
        private void BuildCapOnUpdateSection(VisualElement cap, float lp, ref float y)
        {
            ...
            (_capUpdateBadgeRoot, _capUpdateBadgeBg, _capUpdateBadgeLbl) =
                BuildCapDelegatePicker(lp, y, LeftCapW - lp * 2f);
        ...
        private void BuildCapMoveCloseButtons(VisualElement cap, float lp)
        {
            ...
            if (canMoveUp)
            {
                upBtn.style.color = _theme.CapBtn;
                upBtn.RegisterCallback<MouseEnterEvent>(_ => upBtn.style.color = _theme.SlotHi);
                upBtn.RegisterCallback<MouseLeaveEvent>(_ => upBtn.style.color = _theme.CapBtn);
            }
            else
            {
                upBtn.SetEnabled(false);
                upBtn.style.color = _theme.CapBtnDisabled;
            }
            cap.Add(upBtn);
            ...
            if (canMoveDown)
            {
                downBtn.style.color = _theme.CapBtn;
                downBtn.RegisterCallback<MouseEnterEvent>(_ => downBtn.style.color = _theme.SlotHi);
                downBtn.RegisterCallback<MouseLeaveEvent>(_ => downBtn.style.color = _theme.CapBtn);
            }
            else
            {
                downBtn.SetEnabled(false);
                downBtn.style.color = _theme.CapBtnDisabled;
            }
            cap.Add(downBtn);
            ...
            closeBtn.style.color = _theme.CapBtn;
            ...
            closeBtn.RegisterCallback<MouseLeaveEvent>(_ => closeBtn.style.color = _theme.CapBtn);
            cap.Add(closeBtn);
        }
        ...
        private void AddSlotBox(int i)
        {
            ...
            box.style.backgroundColor = _theme.StripBg;
        ...
        /// <summary>The chrome every left-cap picker shares: the clickable box, the colour chip
        /// that reads bound / unbound / broken, and the clipped method label. Callers add only their
        /// own click and drag-to-pick handlers to the returned root.</summary>
        private (VisualElement root, VisualElement badge, Label label) BuildPickerChrome(
            float x, float y, float width, string unsetText)
        {
            ...
            var label = new Label(unsetText) { pickingMode = PickingMode.Ignore };
        ...
        {
            var (root, badge, label) = BuildPickerChrome(x, y, width, "zero");
        ...
        {
            var (root, badge, label) = BuildPickerChrome(x, y, width, "\u2014 unset \u2014");
        ...
        public void ApplyTheme(JbTheme t)
        {
            ...
            style.unityBackgroundImageTintColor = t.StripSpeckle;
            if (_leftCapEl != null)
            {
            ...
            for (int i = 0; i < _slotBoxes.Count; i++)
            {
                ...
                SetBorder(_slotBoxes[i], border, hi ? 2f : 1f);
                _slotBoxes[i].style.backgroundColor = hi
                    ? (_highlightedDenied ? t.SlotBgDenied : t.SlotBgHi)
                    : t.StripBg;
            }
        }
    }
