...
namespace JuiceBox
{
    ...
    internal static class LayoutBackupManager
    {
        ...
        public static bool RestoreSnapshot(JuiceBoxAnimation anim, int index)
            => RestoreSnapshot(anim, index, out _);

        public static bool RestoreSnapshot(JuiceBoxAnimation anim, int index, out string error)
        {
            error = null;
            if (anim == null || string.IsNullOrEmpty(((IAnimationEditorComponent)anim)._layoutBackupId))
            { error = "This animation has no layout backup id."; return false; }

            string filePath = GetFilePath(anim);
            if (filePath == null || !File.Exists(filePath))
            { error = "No layout backup file was found for this animation."; return false; }

            LayoutBackupFile bf = ReadFile(filePath);
            if (bf.snapshots == null || index < 0 || index >= bf.snapshots.Count)
            { error = "That layout snapshot is no longer available."; return false; }

            string json = bf.snapshots[index].layoutJson;
            if (string.IsNullOrEmpty(json))
            { error = "The layout snapshot is empty or could not be read."; return false; }
    ...
    internal sealed class LayoutRestoreWindow : EditorWindow
    {
        ...
        private void TryRestore(int index, string label)
        {
            ...
            if (!LayoutBackupManager.RestoreSnapshot(_anim, index, out string err))
            {
                _error = err ?? "Could not restore this layout snapshot.";
                Debug.LogWarning("JuiceBox: Failed to restore layout snapshot. " + err);
                return;
            }
