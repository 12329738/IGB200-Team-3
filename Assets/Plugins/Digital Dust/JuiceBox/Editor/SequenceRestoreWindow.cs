...
namespace JuiceBox
{
    internal sealed class SequenceRestoreWindow : EditorWindow
    {
        ...
        private void TryRestore(int index, string label)
        {
            ...
            if (!SequenceBackupManager.RestoreSnapshot(_sequence, index, null, out string err))
            {
                _error = err ?? "Could not restore this snapshot.";
                Debug.LogWarning($"JuiceBox: Failed to restore snapshot for \"{_sequence.Name}\". {err}");
                return;
            }
