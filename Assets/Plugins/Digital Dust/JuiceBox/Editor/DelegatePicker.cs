...
namespace JuiceBox
{
    internal sealed class DelegatePicker : VisualElement
    {
        ...
        private static Color DragHighlight => JbTheme.Default.DragHighlight;
           ...
           ValueSlotRef valueSlot = default, GameObject prePickedTarget = null)
        {
            ...
            catch (System.Exception seedEx)
            {
                UnityEngine.Debug.LogWarning("JuiceBox: delegate picker could not seed from the existing binding (" + seedEx.Message + "); opening unseeded.");
           ...
           Type returnType, Type[] paramTypes, bool strict = false)
        {
            ...
            var goParams = new Type[paramTypes.Length + 1];
            goParams[0] = typeof(GameObject);
            for (int i = 0; i < paramTypes.Length; i++)
                goParams[i + 1] = paramTypes[i];
            ...
            foreach (var type in GetAllowedStaticTypes())
            {
                ...
                for (int m = 0; m < all.Length; m++)
                    if (MatchesStaticSignature(all[m], vd, returnType, paramTypes, goParams, strict))
                        count++;

                if (count == 0) continue;
                var matches = new MethodInfo[count];
                int idx = 0;
                for (int m = 0; m < all.Length; m++)
                    if (MatchesStaticSignature(all[m], vd, returnType, paramTypes, goParams, strict))
                        matches[idx++] = all[m];
        ...
        private static bool MatchesStaticSignature(MethodInfo m, bool voidReturn,
            Type returnType, Type[] paramTypes, Type[] goParams, bool strict)
        {
            if (voidReturn ? m.ReturnType != typeof(void) : m.ReturnType != returnType)
                return false;
            ...
            if (ps.Length == goParams.Length)
            {
                bool ok = true;
                for (int i = 0; i < goParams.Length; i++)
                    if (ps[i].ParameterType != goParams[i]) { ok = false; break; }
                if (ok) return true;
            }
            return false;
        }
        ...
        private static Type TypeOnObject(UnityEngine.Object obj, string methodName)
        {
            ...
            return types.Find(t =>
            {
                ...
                for (int i = 0; i < ms.Length; i++)
                    if (ms[i].Name == methodName) return true;
                return false;
            });
        }
        ...
        private void Info(VisualElement parent, string text)
        {
            ...
            l.style.color = _theme.PortLabel;
