...
namespace JuiceBox
{
    ...
    public static class StandardFunctions
    {
        ...
        // -- GameObject state and lifecycle ---------------------------

        /// <summary>Activates the current GameObject (equivalent to enabling it in the
        /// inspector). Has no effect if it is already active.</summary>
        public static void SetThisGameObjActive(GameObject target)
        {
            target.SetActive(true);
        }

        /// <summary>Deactivates the current GameObject, disabling it and all of its
        /// children until it is reactivated.</summary>
        public static void SetThisGameObjInactive(GameObject target)
        {
            target.SetActive(false);
        }

        /// <summary>Destroys the current GameObject (and all of its children).</summary>
        public static void DestroyThisGameObject(GameObject target)
        {
        ...
        public static class Easing
        {
            ...
            public static float ElasticIn(float time)
            {
                ...
                return -(float)System.Math.Pow(2f, (10f * time) - 10f) *
                    (float)System.Math.Sin(((time * 10f) - 10.75f) * ElasticC4);
            }
            ...
            public static float ElasticOut(float time)
            {
                if (time <= 0f) return 0f;
                if (time >= 1f) return 1f;
                return ((float)System.Math.Pow(2f, -10f * time) *
                    (float)System.Math.Sin(((time * 10f) - 0.75f) * ElasticC4)) + 1f;
            }
            ...
            public static float ElasticInOut(float time)
            {
                if (time <= 0f) return 0f;
                if (time >= 1f) return 1f;
                float wobble = (float)System.Math.Sin(((20f * time) - 11.125f) * ElasticC5);
                if (time < 0.5f)
                    return -((float)System.Math.Pow(2f, (20f * time) - 10f) * wobble) * 0.5f;
                return ((float)System.Math.Pow(2f, (-20f * time) + 10f) * wobble * 0.5f) + 1f;
            }
            ...
            /// <summary>Eases in by first pulling backward before moving forward.</summary>
            public static float BackIn(float time)
            {
                return (BackC3 * time * time * time) - (BackC1 * time * time);
            }
            ...
            {
                time -= 1f;
                return 1f + (BackC3 * time * time * time) + (BackC1 * time * time);
            }

            /// <summary>Eases in and out with a pull back at the start and an overshoot at the finish.</summary>
            public static float BackInOut(float time)
            {
                if (time < 0.5f)
                {
                    float rise = 2f * time;
                    return (rise * rise * (((BackC2 + 1f) * rise) - BackC2)) * 0.5f;
                }
                float fall = (2f * time) - 2f;
                return ((fall * fall * (((BackC2 + 1f) * fall) + BackC2)) + 2f) * 0.5f;
            }
